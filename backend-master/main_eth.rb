...
user_fee_id = DATABASE[:users].insert(eth: '0', eur: '0')

puts

puts '### --- Create new market :'
market = PaymiumMarket::Market::ETHMarket.new(PaymiumMarket::Database::MysqlConnector.new, fee = 0.25, fee_user_id = user_fee_id)
...
user_id_1 = DATABASE[:users].insert(eth: '12', eur: '45000')
user_id_2 = DATABASE[:users].insert(eth: '2', eur: '15000')
...
puts "btc balance"
puts "first user btc balance: #{DATABASE[:users].where(id: user_id_1).first[:eth].to_f}"
puts "second user btc balance: #{DATABASE[:users].where(id: user_id_2).first[:eth].to_f}"
